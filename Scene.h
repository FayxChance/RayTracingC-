    ...
    */
    ...
    struct Scene {
        ...
        rayIntersection(const Ray &ray,
            ...
            float dist = MAXFLOAT;
            Point3 point;
            for (auto it = myObjects.begin(), itE = myObjects.end(); it != itE; it++) {
                if((*it)->rayIntersection(ray,point) < 0){
                    if(dist > distance(ray.origin, point)){
            ...
            return dist == MAXFLOAT ? 1.0f : -1.0f;
        }
